...
// Sale configurations (extracted from existing pages)
const sales = {
  'lake-forest-may-16-17': {
    address: '24668 Jutewood Pl',
    city: 'Lake Forest',
    zip: '92630',
    dates: 'May 16-17, 2026',
    dateDisplay: 'Sat-Sun, May 16-17',
    hours: '8:00 AM - 2:00 PM',
    description: 'Pristine estate sale in Lake Forest featuring quality furnishings, collectibles, and household items.'
  },
  'san-diego-may-16-17': {
    ...
    dateDisplay: 'Sat-Sun, May 16-17',
    hours: '8:00 AM - 2:00 PM',
    description: 'Beautiful estate sale in San Diego with furniture, art, and home decor.'
  },
  'el-cajon-may-16-17': {
    ...
    dateDisplay: 'Sat-Sun, May 16-17',
    hours: '8:00 AM - 2:00 PM',
    description: 'Estate sale in El Cajon featuring furniture, collectibles, tools, and more.'
  },
  'mission-viejo-may-16-17': {
    ...
    dateDisplay: 'Sat-Sun, May 16-17',
    hours: '8:00 AM - 2:00 PM',
    description: 'Estate sale in Mission Viejo with quality furniture, household goods, and decorative items.'
  },
  'escondido-may-16-17': {
    ...
    dateDisplay: 'Sat-Sun, May 16-17',
    hours: '8:00 AM - 2:00 PM',
    description: 'Estate sale in Escondido featuring furniture, artwork, kitchenware, and more.'
  }
};

// Function to extract images array from existing page
function extractImages(htmlPath) {
  const html = fs.readFileSync(htmlPath, 'utf-8');
  const match = html.match(/const images = \[([\s\S]*?)\];/);
  if (!match) {
    console.error(`Could not extract images from ${htmlPath}`);
    return [];
  }
  const imagesStr = match[1];
  const images = imagesStr.match(/'([^']+)'/g).map(s => s.slice(1, -1));
  return images;
}

// Generate HTML template
function generateHTML(sale, images) {
  const mapQuery = encodeURIComponent(`${sale.address}, ${sale.city}, CA ${sale.zip}`);
...
<head>
  ...
  <meta name="description" content="Estate sale at ${sale.address}, ${sale.city}, CA ${sale.zip}. ${sale.dates}, ${sale.hours}.">
...
<body class="bg-gray-50">
  <header class="bg-tlh-dark text-white py-4">
    <div class="max-w-6xl mx-auto px-4 flex justify-between items-center">
      <a href="/"><img src="/images/tlhLOGO.webp" alt="True Legacy Homes" class="h-12 brightness-200"></a>
  ...
  <div class="max-w-6xl mx-auto px-4 py-8">
    <div class="bg-white rounded-xl p-6 shadow-sm mb-6">
      ...
      <p class="text-gray-700">${sale.description}</p>
    </div>
    ...
    <div class="flex flex-col md:grid md:grid-cols-3 gap-6">
      ...
      <div class="md:col-span-2 space-y-6 order-2 md:order-1">
        <div class="bg-white rounded-xl p-6 shadow-sm">
          ...
          <span class="inline-block bg-red-600 text-white px-3 py-1 rounded-full text-sm font-semibold">${sale.dates}</span>
        </div>

        <div class="bg-white rounded-xl p-6 shadow-sm">
          <h2 class="text-2xl font-bold mb-4">Photos <span class="text-gray-400 text-lg font-normal">(${images.length})</span></h2>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm sticky top-4">
          ...
          <div class="space-y-4 text-sm">
            <div class="flex items-start gap-3">
              ...
              <div><p class="font-semibold">Dates</p><p class="text-gray-600">${sale.dateDisplay}</p></div>
            </div>
            <div class="flex items-start gap-3">
              <div class="w-10 h-10 bg-tlh-teal/10 rounded-lg flex items-center justify-center flex-shrink-0">
              ...
              <div><p class="font-semibold">Hours</p><p class="text-gray-600">${sale.hours}</p></div>
            </div>
            <div class="flex items-start gap-3">
              <div class="w-10 h-10 bg-tlh-teal/10 rounded-lg flex items-center justify-center flex-shrink-0">
            ...
            <a href="https://maps.google.com/?q=${mapQuery}" target="_blank" class="block w-full bg-tlh-teal text-white text-center py-3 rounded-lg font-semibold hover:bg-tlh-teal/90 transition">
              Get Directions
            </a>
...
// Process each sale
Object.entries(sales).forEach(([folder, sale]) => {
  const indexPath = path.join(__dirname, 'upcoming-sales', folder, 'index.html');
  
  // Extract images from existing page
  const images = extractImages(indexPath);
  console.log(`${folder}: Found ${images.length} images`);
  
  // Generate new HTML
  const newHTML = generateHTML(sale, images);
